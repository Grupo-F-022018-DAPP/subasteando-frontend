import { Auction } from './auction';

export const AUCTIONS: Auction[] = [
  {
    id: 1;
    title: "Jarron de la dinastia Ming";
    description: "Pero que lindo jarroncito! No puedo creer la belleza. No puedo creer la belleza. No puedo creer la belleza. No puedo creer la belleza.";
    //pictures: string; es una lista
    initialPrice: 1000;
    // fechas
    //startDate: string;
    //endDate: string;
    state: "En progreso";
    // list of biddings
    //biddings: string;
    actualPrice: 1100;
  },
  {
    id: 2;
    title: "Mano de mono";
    description: "No puedo creer la belleza. No puedo creer la belleza. No puedo creer la belleza. No puedo creer la belleza.";
    //pictures: string; es una lista
    initialPrice: 1000;
    // fechas
    //startDate: string;
    //endDate: string;
    state: "En progreso";
    // list of biddings
    //biddings: string;
    actualPrice: 1100;
  },
  {
    id: 3;
    title: "Cuadro de Picasso";
    description: "Y esta rosa?";
    //pictures: string; es una lista
    initialPrice: 1000;
    // fechas
    //startDate: string;
    //endDate: string;
    state: "En progreso";
    // list of biddings
    //biddings: string;
    actualPrice: 1100;
  }
];
